...
class CanuUtil:
    ...
    def _upload_assembly(self, workspace_name, contigs_file,
        ...
        """
        # Build a human-readable description of the parameters used
        param_desc = "; ".join(
            "{}={}".format(k, v)
            for k, v in sorted(params.items())
            if k not in ("workspace_name",) and v not in (None, "", 0)
        )
        ...
        result = self.au.save_assembly_from_fasta({
            ...
            "reads_source": {"reads_ref": reads_ref},
            "provenance": [{
                "service":        "kb_canu",
                "service_ver":    "0.0.1",
                "method":         "run_kb_canu",
                "method_params":  [params],
                "input_ws_objects": [reads_ref],
                "description": (
                    "Genome assembly generated by Canu v2.2 "
                    "({})".format(param_desc)
                ),
            }],
        })
